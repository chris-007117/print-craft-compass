...
import { SiteSearch, SearchTrigger, useSearchHotkey } from "./SiteSearch";
      ...
      <header className={`sticky top-0 z-50 ${isHome ? 'bg-charcoal/90 backdrop-blur-md' : 'bg-charcoal'} border-b border-bone/10`}>
        <div className="container-x flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <SearchTrigger onClick={() => setSearchOpen(true)} variant="pill" />
            <div className="xl:hidden">
              <SearchTrigger onClick={() => setSearchOpen(true)} variant="icon" />
            </div>
            <a href="tel:+16025551987" className="hidden md:flex items-center gap-2 text-bone/70 hover:text-bone text-xs">
              <Phone className="h-3.5 w-3.5" /> 602.555.1987
            </a>
        ...
        {open && (
          <div className="lg:hidden bg-charcoal border-t border-bone/10">
            <nav className="container-x py-6 flex flex-col gap-1">
              <div className="mb-4">
                <SearchTrigger onClick={() => { setOpen(false); setSearchOpen(true); }} variant="full" />
              </div>
              {nav.map((n) => (
                <NavLink
                  key={n.to}
              ...
              <a href="tel:+16025551987" className="mt-3 text-bone/70 text-sm">602.555.1987</a>
            </nav>
          </div>
        )}
      </header>
